...
describe("config", () => {
  ...
  describe.skipIf(skipTests)("--schema-url", () => {
    test("it should allow no file input", () => {
      expectConfiguration("schemaUrl").with(undefined, "SCHEMA_URL", "-s", "--schema-url").toBe("./schema.sql");
    });

    test("it should allow a string input", () => {
      expectConfiguration("schemaUrl").with("file.sql", "SCHEMA_URL", "-s", "--schema-url").toBe("file.sql");
    });
  });

  describe.skipIf(skipTests)("--public-key | --auth-key | --host | --database | --username | --password", () => {
  ...
  describe.skipIf(skipTests)("--create-database", () => {
    test("it should allow boolean values", () => {
      expectConfiguration("createDatabase").with("true", "CREATE_DATABASE", "--create-database").toBe(true);
      expectConfiguration("createDatabase").with("false", "CREATE_DATABASE", "--create-database").toBe(false);
    });

    test("it should fail if any other value is passed in", () => {
      expectConfiguration("createDatabase").with("random", "CREATE_DATABASE", "--create-database").toThrow();
    });
  ...
  describe.skipIf(skipTests)("--async-insert | --wait-for-insert", () => {
    test("it should allow 0 or 1 as an input", () => {
      ...
      expectConfiguration("waitForInsert").with("0", "WAIT_FOR_INSERT", "--wait-for-insert").toBe(0);

      expectConfiguration("asyncInsert").with("1", "ASYNC_INSERT", "--async-insert").toBe(1);
      expectConfiguration("waitForInsert").with("1", "WAIT_FOR_INSERT", "--wait-for-insert").toBe(1);
    });

    test("it should fail on any other input", () => {
      expectConfiguration("asyncInsert").with("2", "ASYNC_INSERT", "--async-insert").toThrow();
      expectConfiguration("waitForInsert").with("2", "WAIT_FOR_INSERT", "--wait-for-insert").toThrow();
    });
  });
  ...
  describe.skipIf(skipTests)("--queue-limit | --queue-concurrency", () => {
    test("it should allow to specify a positive number", () => {
      ...
      expectConfiguration("queueConcurrency").with("10", "QUEUE_CONCURRENCY", "--queue-concurrency").toBe(10);
    });

    test("it should fail on invalid number", () => {
      expectConfiguration("queueLimit").with("-1", "QUEUE_LIMIT", "--queue-limit").toThrow();
      expectConfiguration("queueConcurrency").with("-1", "QUEUE_CONCURRENCY", "--queue-concurrency").toThrow();
    });
  });
});
